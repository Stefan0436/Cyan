				...
				for (MethodNode meth : transformerNode.methods) {
					...
					if (AnnotationInfo.isAnnotationPresent(Erase.class, meth)) {
						...
						targetNode.maxLocals = meth.maxLocals
								+ (!Modifier.isStatic(targetNode.access) ? (Modifier.isStatic(meth.access) ? 0 : 1)
										: 0);
						...
						if (AnnotationInfo.getAnnotation(InjectAt.class, meth).get("targetCall") != null) {
							FluidMethodInfo targetInfo = FluidMethodInfo.create(
									AnnotationInfo.getAnnotation(InjectAt.class, meth).<String>get("targetCall"));
						...
						if (targetNode.localVariables != null && meth.localVariables != null) {
							if (meth.localVariables.size() > actualParams.length) {
								...
								for (int i = actualParams.length
										+ (Modifier.isStatic(meth.access) ? 0 : 1); i < meth.localVariables
												.size(); i++) {
									LocalVariableNode lvn = meth.localVariables.get(i);
									if (Fluid.parseDescriptor(lvn.desc)
											.equals(transformerNode.name.replaceAll("/", "."))) {
									...
									if (targetNode.localVariables.size() == 0) {
										...
										for (int i = 0; i < mthLocalVars.length; i++) {
											mthLocalVars[i].index = addVarStart
													+ (Modifier.isStatic(meth.access) ? 0 : 1) + i;
										}
						...
						int offset = at.get("offset", 0);
						if (!lineless && injectNode == null && targetMethName == null) {
							throw new RuntimeException("Unable to find LabelNode offset " + offset + " for class "
									+ clName + ", transformer cannot be applied, method: " + meth.name
						...
						if (cls.methods.stream()
							...
							if (!meth.name.equals("<clinit>"))
								throw new RuntimeException("Unable to add method " + methNameFinal + " to class "
										+ clName + " as a method with the same signature already exists, transformer: "
										+ typeName);
							else
								continue;
						}
