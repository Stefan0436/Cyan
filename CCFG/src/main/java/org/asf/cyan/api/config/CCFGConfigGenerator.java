...
/**
 * 
 * Experimental CCFG Generator, <b>AVOID USAGE, NO SUPPORT FOR VALUE OVERWRITING YET</b>
...
public class CCFGConfigGenerator<T extends Configuration<T>> extends CCFGGetPropAction<Configuration<T>> {
	...
	public CCFGConfigGenerator(Configuration<T> conf, boolean addAll) {
		this.conf = conf;
		this.addAll = addAll;
	}
		...
		if (key == null) {
			...
			if (!pref.isEmpty()) {
				...
				for (String line : pref.split("\n")) {
					if (newPref.isEmpty()) newPref = "# "+line;
					else newPref += System.lineSeparator()+"# "+line;
				}
				return newPref;
			} else return null;
		}
		...
		if (key == null) {
			...
			if (!suff.isEmpty()) {
				...
				for (String line : suff.split("\n")) {
					if (newSuff.isEmpty()) newSuff = "# "+line;
					else newSuff += System.lineSeparator()+"# "+line;
				}
				return newSuff;
			} else return null;
		}
	...
	public String[] keys() {
		if (!addAll) {
			...
			for (Field f : conf.getClass().getFields()) {
				...
				if (!conf.isExcluded(f) && conf.hasChanged(f, conf.getProp(f))) {
					keys.add(f.getName());					
				} else if (!conf.isExcluded(f) && conf.isOptional(f) && !conf.isPresent(f) && (conf.getKeyHeaderComment(f) != "" || conf.getKeyFooterComment(f) != "")) {
			...
			for (Field f : conf.getClass().getFields()) {
				f.setAccessible(true);
				try {
					if (!conf.isExcluded(f) && f.get(conf) != null) keys.add(f.getName());
				} catch (IllegalArgumentException | IllegalAccessException e) {
				}
			}
			return keys.toArray(t->new String[t]);
	...
	public String postProcess(String key, String entry) {
		if (disabled.contains(key)) {
			...
			for (String line : entry.split(System.lineSeparator())) {
				if (newEntry.isEmpty()) newEntry = "# "+line;
				else newEntry += System.lineSeparator() + "# "+line;
			}
			return newEntry;
		}
		return entry;
