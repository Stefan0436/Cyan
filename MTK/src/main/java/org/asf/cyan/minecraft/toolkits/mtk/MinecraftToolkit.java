...
/**
 * 
 * Main Minecraft toolkit: resolve the minecraft version list
 * 
...
public class MinecraftToolkit extends CyanComponent {
	...
	protected static void initComponent() {
		trace("INITIALIZE Main Minecraft Toolkit, caller: " + KDebug.getCallerClassName());
	}
	...
	public static void resetServerConnectionState() {
		trace("RESET server connection state, caller: " + KDebug.getCallerClassName());
		_connected = getServerConnection();
		if (!_connected) {
			trace("STATE of connection is " + _connected + ", sending warning, caller: " + KDebug.getCallerClassName());
	...
	public static boolean hasMinecraftDownloadConnection() {
		trace("GET server connection, caller: " + KDebug.getCallerClassName());
		if (!_connected) {
			trace("RETURN false, previous connection state was false, caller: " + KDebug.getCallerClassName());
			return false;
		} else {
			trace("RETURN current connection state, check state, caller: " + KDebug.getCallerClassName());
			resetServerConnectionState();
			trace("RETURN connection state " + _connected + ", caller: " + KDebug.getCallerClassName());
			return _connected;
		}
	}
	...
	public static boolean getServerConnection() {
		try {
			trace("PARSE server connection test URL, caller: " + KDebug.getCallerClassName());
			URL u = new URL(version_manifest_url);
			trace("OPEN connection to network test URL, caller: " + KDebug.getCallerClassName());
			URLConnection uc = u.openConnection();
			trace("CONNECT to server test URL, caller: " + KDebug.getCallerClassName());
			uc.connect();
			trace("RETURN true, all passed, caller: " + KDebug.getCallerClassName());
			return true;
			...
			trace("RETURN false, IOException, message: " + e.getMessage() + ", caller: " + KDebug.getCallerClassName());
			return false;
		}
	}
	...
	public static void resolveVersions() {
		trace("RESOLVE version list, no arguments, caller: " + KDebug.getCallerClassName());
	...
	public static void resolveVersions(boolean noclear) {
		trace("RESOLVE version list, argument noclear set to: " + noclear + ", caller: " + KDebug.getCallerClassName());
		if (hasMinecraftDownloadConnection()) {
			trace("HAS server connection, resolving list, caller: " + KDebug.getCallerClassName());
			try {
				if (!noclear)
					available_versions.clear();
				info("Downloading manifest from " + version_manifest_url + "...");
				trace("OPEN server connection, PARSE URL, caller: " + KDebug.getCallerClassName());
				URL u = new URL(version_manifest_url);
				trace("OPEN stream to server, caller: " + KDebug.getCallerClassName());
				...
				reader.close();
				trace("CREATE Gson object, caller: " + KDebug.getCallerClassName());
				Gson gson = new Gson();
				info("Reading manifest...");

				trace("CREATE map for latest version information, caller: " + KDebug.getCallerClassName());
				@SuppressWarnings("unchecked")
				Map<String, String> latest = gson.fromJson(json.get("latest"), Map.class);
				String latestRelease = "";
				String latestSnapshot = "";
				trace("CHECK map for release version, caller: " + KDebug.getCallerClassName());
				...
				trace("CREATE DateFormat object, caller: " + KDebug.getCallerClassName());
				info("Adding versions...");
				trace("LOOP through entries, caller: " + KDebug.getCallerClassName());
				for (JsonElement entry : json.get("versions").getAsJsonArray()) {
					trace("CREATE map for version info, caller: " + KDebug.getCallerClassName());
					@SuppressWarnings("unchecked")
					Map<String, String> ver = gson.fromJson(entry, Map.class);

					trace("SET id of version info to " + ver.get("id") + ", caller: " + KDebug.getCallerClassName());
					...
					trace("PARSE MinecraftVersionInfo object, caller: " + KDebug.getCallerClassName());
					...
					if (versionInfo.getVersion().equalsIgnoreCase(latestRelease)) {
						trace("MATCHED latest release version string, caller: " + KDebug.getCallerClassName());
						...
						trace("MATCHED latest snapshot version string, caller: " + KDebug.getCallerClassName());
					...
					trace("ADD MinecraftVersionInfo object to the available_versions map, version: "
							+ versionInfo.getVersion() + ", caller: " + KDebug.getCallerClassName());
				...
				// I know, this will ignore all exceptions, which is exactly what i want
			}
		} else {
			trace("NO CONNECTION to server, cancel task, caller: " + KDebug.getCallerClassName());
			warn("No connection to the manifest server, unable to load version manifest.");
		}
	}
}
