...
public class MinecraftVersionToolkit extends CyanComponent {
	...
	public static void setManifesURL(String url) throws IOException
	{
		try {
			...
			reader.close();
			Gson gson = new Gson();
			if (!json.has("latest")) throw new IOException("Member 'latest' could not be found");
			if (!json.has("versions")) throw new IOException("Member 'versions' could not be found");
			if (!json.get("versions").isJsonArray()) throw new IOException("Member 'versions' is not a JSON array.");
			...
			for (JsonElement entry : json.get("versions").getAsJsonArray()) {
				...
				if (!ver.containsKey("id")) throw new IOException("A version entry was found without the 'id' key");
				if (!ver.containsKey("type")) throw new IOException("A version entry was found without the 'type' key, ID: "+ver.get("id"));
				if (!ver.containsKey("url")) throw new IOException("A version entry was found without the 'url' key, ID: "+ver.get("url"));
				if (!ver.containsKey("releaseTime")) throw new IOException("A version entry was found without the 'releaseTime' key, ID: "+ver.get("releaseTime"));
			}
			u = null;
	 ...
	 * @return MinecraftVersionInfo object representing the latest release (null if Latest_release is empty)
	 */
	...
	/**
	 * Return the latest Minecraft release version
	 * @return MinecraftVersionInfo object representing the latest release (null if Latest_snapshot is empty)
	 */
	 ...
	 * @return The MinecraftVersionInfo object representing the version, null if not found
	 */
